-- =====================================================
-- MOTOR TESTS (20 tests with complete fields)
-- =====================================================
...
((SELECT id FROM test_types WHERE test_name = 'Insulation Resistance' AND asset_type = 'motor'), 'temperature', 'Temperature', '?C', 'number', false, 3);

-- 3. Polarization Index - 2 fields
INSERT INTO test_field_definitions (test_type_id, field_name, display_name, unit, data_type, is_required, display_order) VALUES
((SELECT id FROM test_types WHERE test_name = 'Polarization Index' AND asset_type = 'motor'), 'pi', 'Polarization Index', NULL, 'number', true, 1),
((SELECT id FROM test_types WHERE test_name = 'Polarization Index' AND asset_type = 'motor'), 'dar', 'Dielectric Absorption Ratio', NULL, 'number', false, 2);

-- 4. DC Hi-Pot Test - 3 fields
INSERT INTO test_field_definitions (test_type_id, field_name, display_name, unit, data_type, is_required, display_order) VALUES
((SELECT id FROM test_types WHERE test_name = 'DC Hi-Pot Test' AND asset_type = 'motor'), 'test_voltage', 'Test Voltage', 'kV', 'number', true, 1),
((SELECT id FROM test_types WHERE test_name = 'DC Hi-Pot Test' AND asset_type = 'motor'), 'leakage_current', 'Leakage Current', '?A', 'number', true, 2),
((SELECT id FROM test_types WHERE test_name = 'DC Hi-Pot Test' AND asset_type = 'motor'), 'duration', 'Test Duration', 'min', 'number', false, 3);

-- 5. AC Hi-Pot Test - 2 fields
INSERT INTO test_field_definitions (test_type_id, field_name, display_name, unit, data_type, is_required, display_order) VALUES
((SELECT id FROM test_types WHERE test_name = 'AC Hi-Pot Test' AND asset_type = 'motor'), 'withstand_voltage', 'Withstand Voltage', 'kV', 'number', true, 1),
((SELECT id FROM test_types WHERE test_name = 'AC Hi-Pot Test' AND asset_type = 'motor'), 'leakage_current', 'Leakage Current', 'mA', 'number', true, 2);
...
-- 19. Current Signature Analysis (CSA) - 3 fields
INSERT INTO test_field_definitions (test_type_id, field_name, display_name, unit, data_type, is_required, display_order) VALUES
...
INSERT INTO test_field_definitions (test_type_id, field_name, display_name, unit, data_type, is_required, display_order) VALUES
((SELECT id FROM test_types WHERE test_name = 'Thermography' AND asset_type = 'motor'), 'hotspot_temp', 'Hotspot Temperature', '?C', 'number', true, 1),
((SELECT id FROM test_types WHERE test_name = 'Thermography' AND asset_type = 'motor'), 'ambient_temp', 'Ambient Temperature', '?C', 'number', false, 2),
((SELECT id FROM test_types WHERE test_name = 'Thermography' AND asset_type = 'motor'), 'delta_t', 'Temperature Difference', '?C', 'number', true, 3);

SELECT 'MOTOR: 20 tests, ' || COUNT(*) || ' fields defined' as status FROM test_field_definitions WHERE test_type_id IN (SELECT id FROM test_types WHERE asset_type = 'motor');
